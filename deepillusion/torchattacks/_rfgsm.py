...
def RFGSM(net, x, y_true, data_params, attack_params, loss_function="cross_entropy", verbose=False):
    ...
    if attack_params["norm"] == "inf":
        e = torch.rand_like(x) * 2 * \
            attack_params['eps'] - attack_params['eps']
    else:
        e = 2 * torch.rand_like(x) - 1
        e = e * attack_params["eps"] / \
            e.view(x.shape[0], -1).norm(p=int(attack_params["norm"]),
                                        dim=-1).view(-1, 1, 1, 1)

    # Clip perturbation before gradient calculation to get best performance
    e.data = torch.max(
        torch.min(e, data_params["x_max"] - x), data_params["x_min"] - x)
    ...
    if loss_function == "cross_entropy":
        ...
        wrong_logit = ((1 - y_true_onehot) * y_hat -
                       1e4 * y_true_onehot).max(dim=1)[0]
    ...
    if verbose:
        ...
        if max_attack_for_each_image.min() <= 0:
            warn("Gradient Masking is happening for some images!!!!!",
                 GradientMaskingWarning)
...
def RFGM(net, x, y_true, attack_params, data_params, loss_function="cross_entropy", verbose=False):
    """
    Description: Random + Fast Gradient Method (without sign gives gradients as it is)
    Input :
        net : Neural Network            (torch.nn.Module)
        x : Inputs to the net           (Batch)
        y_true : Labels                 (Batch)
