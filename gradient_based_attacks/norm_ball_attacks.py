...
def RandomFastGradientSignMethod(net, x, y_true, data_params, attack_params):
    """
    Input :
...
def FastGradientSignMethod(net, x, y_true, eps, data_params, norm="inf"):
    """
    Input :
        net : Neural Network                                        (torch.nn.Module)
        x : Inputs to the net                                       (Batch)
...
def ProjectedGradientDescent(net, x, y_true, data_params, attack_params, verbose=True):
    """
    Input :
        net : Neural Network            (torch.nn.Module)
        x : Inputs to the net           (Batch)
